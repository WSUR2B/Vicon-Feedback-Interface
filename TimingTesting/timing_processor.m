...
% timing = readtable("recording_timing_log_1_untrimmed.csv");
% timing = readtable("recording_timing_log_2_untrimmed.csv");
% timing = readtable("recording_timing_log_3_untrimmed.csv");

% timing = readtable("timing_log_went_from_live_to_rec.csv");
% timing = readtable("timing_log_Fxpl_Str_Exp_Lag_SExp_SStr.csv");

timing = readtable("timing_log_recording_feedback_untrimmed.csv");  end_time_margin_remove_s = 10;
% timing = readtable("timing_log_recording_streaming_untrimmed.csv"); end_time_margin_remove_s = 15;
% timing = readtable("timing_log_streaming_untrimmed.csv"); end_time_margin_remove_s = 1;
...
dt_s1_to_s1 = diff(timing.x1_sdk_update_loop_start);
...
dt_sdk = dt_frame_waiting+dt_sdk_device_data_fetch+dt_sdk_labeled_markers_fetch+dt_sdk_unlabeled_markers_fetch+dt_sdk_update_subject_fetchNcompute;
...
dt_p = dt_clean_plot_save_to_csv_get_active_subject+dt_update_plotting+dt_update_exporting+dt_update_streaming+dt_update_feedback;

total_frame_age_at_feedback = timing.x19_update_feedback_end - timing.x2_sdk_update_loop_got_frame;


%Return the total latency in seconds introduced at various stages of the real-time pipeline
%marker move -> sdk frame read
% print_statistics_row_ms("GetLatencyTotal                                    ", timing.GetLatencyTotal*1000)
...
print_statistics_row_ms("S:                                                 ", (dt_sdk)*1000)
print_statistics_row_ms("S-S1:                                              ", (dt_sdk - dt_frame_waiting)*1000)
print_statistics_row_ms("S1 to S1:                                          ", (dt_s1_to_s1)*1000)
print_statistics_row_ms("S1 to S1 FPS:                                      ", 1./dt_s1_to_s1)
...
print_statistics_row_ms("P:                                                 ", (dt_p)*1000)
print_statistics_row_ms("P-P1:                                              ", (dt_p-dt_clean_plot_save_to_csv_get_active_subject)*1000)

disp(' ')

print_statistics_row_ms("from_sdk_frame_got_to_feedback_end_ms              " ,total_frame_age_at_feedback * 1000)
print_statistics_row_ms("marker_move_to_frame_got_ms (GetLatencyTotal)      " ,timing.GetLatencyTotal * 1000)
print_statistics_row_ms("marker_move_to_feedback_end_ms                     " ,(timing.GetLatencyTotal + total_frame_age_at_feedback) * 1000)



hold on
plot(t(1:end-1), 100./diff(timing.frameNumber), '*b')
plot(t(1:end-1), 1./dt_s1_to_s1, '+r')

plot(t(1:end-1), dt_s1_to_s1*1000)
plot(t, dt_sdk*1000)
plot(t, (dt_sdk - dt_frame_waiting)*1000)

plot(t, dt_p * 1000, '--')
plot(t, (dt_p-dt_clean_plot_save_to_csv_get_active_subject)*1000, '--')

plot(t, total_frame_age_at_feedback*1000, 'k')

legend( ...
        'vicon_frames_com_fps', ...
        'sdk_thread_fps', ...
        'dt_s1_to_s1', ...
        'dt_sdk_sum', ...
        'dt_s_minus_s1', ...
...
function print_statistics_row_ms(Name, x)
    ...
    fprintf('%-20s | min: %9.4f | med: %9.4f | max: %9.4f | mean: %9.4f | std: %9.4f | p99: %9.4f\n', ...
